  ...
  if (!LIMIT) {
    ...
    try {
      ...
      const cryptos = data.reduce((resultArray, currentElement) => {
        ...
        coinInfo.price = currentElement.quote.USD.price ;
        coinInfo.volumeP = currentElement.quote.USD['percent_change_24h'];
        // coinInfo.volumn24 = currentElement.quote.BTC["volume_24h"]
...
router.get('/crypto/transaction', async (req, res) => {
  try {
    const apiResponse = await axios.get(`${process.env.CMC_API_URL_V1}/exchange/listings/latest`, {headers: CMC_HEADER});
    const data = apiResponse.data.data;
